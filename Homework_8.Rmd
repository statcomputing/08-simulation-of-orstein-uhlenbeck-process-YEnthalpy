...
\subsection{Problem 1}
The distribution for $N(5)$ should be
\begin{equation*}
\begin{aligned}
P\left\lbrace N(5) = k\right\rbrace 
= P\left\lbrace N(0, 5]  = k\right\rbrace \\
&= \frac{[\Lambda(0,5)] ^ n}{n!}\exp(-\Lambda(0,5)),
\end{aligned}
\end{equation*}
where
\begin{equation*}
\begin{aligned}
\Lambda(0,5) &= \int_0^5 \lambda(t)\text{d}t \\
&= \int_0^5 \sqrt{t} + \exp(-t) \sin(2\pi t) \text{d}t \\
&= 7.608.
\end{aligned}
\end{equation*}
The R code for calculating the integration is shown below.
```{r}
inten <- function(t) {
  sqrt(t) + exp(-t) * sin(2 * pi * t)
}
int.mea <- round(integrate(inten, 0, 5)$value, 3)
cat("The intensity measure is ", int.mea, ".", sep = "")
```
Thus, we can conclude that
\begin{equation*}
\begin{aligned}
P\left\lbrace N(5) = n \right\rbrace
&= \frac{7.608 ^ n}{n!}\exp(-7.608),\\
N(5) &\sim \text{Poission(7.608)}.
\end{aligned}
\end{equation*}

\subsection{Problem 2}
The function to simulate from this Poisson process is shown below.
```{r}
t <- seq(0, 5, 0.01)
intmax <- max(inten(t))
gen.poi <- function(control = list(intensity = inten, 
                                   intmax = intmax, tmax = 5)) {
  n <- rpois(1, control$intmax)
  tt <- runif(n, 0, control$tmax)
  u <- runif(n)
  accept <- u < inten(tt) / control$intmax
  sort(tt[accept])
}
```

\subsection{Problem 3}
Generate events 1000 times and then draw plots.
```{r}
ran.poi <- unlist(replicate(1000, gen.poi()))
y <- inten(t) / int.mea
plot(density(ran.poi, from = 0, to = 5), xlab = "t", ylab = "Density", 
     main = "Density plots", col = "red", lwd = 2, lty = 3, ylim = c(0, 0.3))
lines(y ~ t, col = "blue", lwd = 2, lty = 2)
legend("topleft", c("Kernal", "True"), lty = c(3, 2), 
